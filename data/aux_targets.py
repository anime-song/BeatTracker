...
def _hz_to_midi(freq_hz: torch.Tensor) -> torch.Tensor:
    return 69.0 + (12.0 * torch.log2(freq_hz / 440.0))
...
class StemAuxTargets:
    ...
    bass_harmonic_change_targets: torch.Tensor
...
class StemAuxTargetBuilder:
    ...
    def build(self, waveform: torch.Tensor, valid_frames: int) -> StemAuxTargets:
        ...
        bass_low_flux_targets, bass_harmonic_change_targets = (
            self._compute_bass_aux_targets(
                waveform=waveform,
                valid_frames=valid_frames,
            )
        )
        return StemAuxTargets(
            ...
            bass_harmonic_change_targets=bass_harmonic_change_targets,
        )
    ...
    def _compute_bass_aux_targets(
        ...
        # bass stem から low-band flux と harmonic change を作る。
        empty = torch.zeros(
            self.target_num_frames,
            dtype=torch.float32,
            device=waveform.device,
        ...
        chroma_mapping = F.one_hot(
            torch.remainder(torch.round(_hz_to_midi(chroma_freqs)), 12).long(),
            num_classes=12,
        ...
        harmonic_change = _minmax_normalize_1d(harmonic_change)
        return (
            self._pad_to_target_length(low_band_flux, valid_frames),
            self._pad_to_target_length(harmonic_change, valid_frames),
        )
