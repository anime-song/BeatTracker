...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default="./dataset/meter_dataset/audios",
        help="入力の [.wav, .mp3] または [.wav, .mp3] を含むフォルダ",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default="./dataset/meter_dataset/songs_separated",
        help="ステム出力先フォルダ",
    )
    parser.add_argument(
        "--resample-rate",
    ...
    if input_path.is_file():
        ...
        target_files = sorted(
            [
                ...
                if p.is_file()
                and (p.suffix.lower() == ".wav" or p.suffix.lower() == ".mp3")
            ]
        )
    else:
    ...
    separate_config = SeparationConfig(skip_existing=True, batch_size=args.batch_size)
    ...
    if not separated_paths:
        print(
            "新規のステム出力が見つからなかったため、リサンプリングをスキップしました。"
        )
        return
    ...
    with ProcessPoolExecutor(max_workers=worker_count) as executor:
        futures = [
            executor.submit(resample_in_place, path, args.resample_rate)
