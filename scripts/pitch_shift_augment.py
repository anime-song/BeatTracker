...
try:
    ...
    try:
        ...
        print(
            "エラー: librosaがインストールされていません。'pip install librosa' を実行してください。"
        )
        exit()
...
def pitch_shift_file(args):
    ...
    try:
        ...
        if USE_PYRUBBERBAND:
            ...
            if y.ndim > 1:  # ステレオか多チャンネル音源の場合
                y_shifted = librosa.effects.pitch_shift(
                    y.T, sr=sr, n_steps=float(semitone)
                ).T
            else:  # モノラル音源の場合
                y_shifted = librosa.effects.pitch_shift(
                    y, sr=sr, n_steps=float(semitone)
                )
        ...
        print(
            f"✗ エラー発生: {os.path.basename(input_path)} (semitone: {semitone}) - {e}"
        )
...
if __name__ == "__main__":
    ...
    if not original_files:
        print(
            f"フォルダ内に処理対象のオリジナル.wavファイルが見つかりませんでした: {target_dir}"
        )
    else:
        # 実行するタスクのリストを作成
        # (ファイルパス, 半音数) のタプルのリスト
